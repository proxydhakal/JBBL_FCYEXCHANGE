...
def generate_pdf_receipt(request, id):
    ...
    table_data = [
        ['Currency', 'Deno Details']
    ]

    subtable_data = [
        ['deno', 'unit', 'rate', 'equivalent_npr']
    ]
    ...
    # Loop through each currency and its details
    for currency, details in data_by_currency.items():
        # Add a row for the currency
        table_data.append([currency[:3], subtable_data])

        # Loop through each detail of the currency
        for detail in details:
            # Add a row for each detail
            subtable_data.append([detail['deno'], detail['unit'], detail['rate'], detail['equivalent_npr']])
    
    table = Table(table_data,subtable_data, colWidths=100)
